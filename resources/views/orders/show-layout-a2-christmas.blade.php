            ...
            <div class="space-y-6" id="order-a2-card-container">
                @forelse($itemsCollection as $card)
                    @php
                        ...
                        $christmasChartData = null;
                        if (isset($card['context_data']['christmas_comparison']) && $card['context_data']['christmas_comparison']) {
                            $christmasWindows = $card['context_data']['christmas_comparison']['christmas_windows'] ?? [];
                            $christmasChartData = [];
                            foreach ($christmasWindows as $window) {
                                $christmasChartData[] = [
                                    'year' => $window['year'],
                                    'labels' => array_column($window['weekly_breakdown'], 'label'),
                                    'sales' => array_column($window['weekly_breakdown'], 'units'),
                                ];
                            }
                        }
                        ...
                        $chartPayload = [
                            ...
                            'christmasData' => $christmasChartData,
                        ];
                    @endphp
                    <article
                            ...
                            <div class="flex-1 p-6 md:pl-8">
                                <div class="flex flex-col gap-6 lg:flex-row lg:items-start">
                                    <div class="rounded-xl border border-slate-200 bg-slate-50/60 p-4 lg:flex-1">
                                        ...
                                        @php
                                            $christmasData = $card['context_data']['christmas_comparison'] ?? null;
                                            $stats = $christmasData['stats'] ?? null;
                                            $isChristmasHigher = $stats && ($stats['selected_mode'] ?? '') === 'christmas';
                                        @endphp

                                        @if($christmasData && $stats)
                                            <!-- Christmas Comparison View -->
                                            <div class="mt-4 grid grid-cols-2 gap-3 p-3 bg-slate-100 rounded-lg border border-slate-300">
                                                <!-- Regular Column -->
                                                <div class="border-r border-slate-300 pr-3">
                                                    <div class="text-xs font-semibold uppercase text-slate-500 mb-2">
                                                        Recent ({{ $order->sales_history_weeks }}w)
                                                    </div>
                                                    <div class="space-y-1">
                                                        <div>
                                                            <div class="text-[10px] text-slate-600">Weekly avg</div>
                                                            <div class="text-lg font-bold text-blue-600">
                                                                {{ number_format($stats['regular_weekly_avg'], 1) }}
                                                            </div>
                                                        </div>
                                                        <div>
                                                            <div class="text-[10px] text-slate-600">Suggested</div>
                                                            <div class="text-base font-semibold {{ !$isChristmasHigher ? 'text-green-600' : 'text-slate-600' }}">
                                                                {{ number_format($stats['regular_suggested'], 0) }}
                                                                @if(!$isChristmasHigher)
                                                ...
                                                <div class="pl-3">
                                                    <div class="text-xs font-semibold uppercase text-slate-500 mb-2">
                                                        Christmas ({{ implode(', ', $christmasData['years']) }})
                                                    </div>
                                                    <div class="space-y-1">
                                                        <div>
                                                            <div class="text-[10px] text-slate-600">Weekly avg</div>
                                                            <div class="text-lg font-bold text-purple-600">
                                                                {{ number_format($stats['christmas_weekly_avg'], 1) }}
                                                            </div>
                                                        </div>
                                                        <div>
                                                            <div class="text-[10px] text-slate-600">Suggested</div>
                                            ...
                                            @if(abs($stats['delta']) > 5)
                                                <div class="mt-2 p-2 rounded-lg text-xs {{ $stats['delta'] > 0 ? 'bg-orange-50 text-orange-800 border border-orange-200' : 'bg-green-50 text-green-800 border border-green-200' }}">
                                                    <div class="flex items-center gap-2">
                                                        <span class="text-lg">{{ $stats['delta'] > 0 ? '⬆️' : '⬇️' }}</span>
                                                        <div>
                                                            <strong>{{ abs($stats['delta']) }} units difference</strong>
                                                            <br>Using {{ $isChristmasHigher ? 'Christmas' : 'regular' }} (higher)
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
                                            ...
                                            <div class="mt-3 flex gap-4 text-xs">
                                                <div>
                                                    <div class="text-slate-500 uppercase tracking-wide text-[10px]">Current</div>
                                                    <div class="text-base font-semibold text-rose-600">{{ number_format($card['stats']['current_stock']) }}</div>
                                                </div>
                                                <div>
                                                    <div class="text-slate-500 uppercase tracking-wide text-[10px]">After order</div>
                                                    <div class="text-base font-semibold text-emerald-600" id="after-stock-value-{{ $card['id'] }}">{{ number_format($card['stats']['after_stock']) }}</div>
                                                </div>
                                            </div>
                                        @else
                                            <!-- Regular View (No Christmas Data) -->
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const renderChart = (canvas) => {
                ...
                // Build labels and datasets with Christmas data if available
                let labels = [...payload.labels, 'Current', 'After'];
                let sales = [...payload.sales, null, null];
                const datasets = [];

                // Add regular sales dataset
                datasets.push({
                    type: 'line',
                    label: 'Recent sales',
                    data: sales,
                ...
                // Add Christmas datasets if available
                if (payload.christmasData && payload.christmasData.length > 0) {
                    const christmasColors = ['rgb(168, 85, 247)', 'rgb(236, 72, 153)', 'rgb(59, 130, 246)'];

                    payload.christmasData.forEach((christmasWindow, index) => {
                        // Add separator and Christmas labels
                        labels.push('—');
                        const christmasLabels = christmasWindow.labels.map(l => `'${String(christmasWindow.year).slice(-2)} ${l}`);
                        labels.push(...christmasLabels);

                        // Extend sales data with nulls
                        sales.push(...Array(1 + christmasLabels.length).fill(null));

                        // Create dataset for this Christmas year
                        const christmasData = [
                            ...Array(payload.labels.length + 3 + (index * (christmasWindow.labels.length + 1))).fill(null),
                            ...christmasWindow.sales
                        ];

                        // Pad to match total length
                        while (christmasData.length < labels.length) {
                            christmasData.push(null);
                        }

                        datasets.push({
                            type: 'line',
                            label: `Christmas ${christmasWindow.year}`,
                            data: christmasData,
                            borderColor: christmasColors[index % christmasColors.length],
                            backgroundColor: christmasColors[index % christmasColors.length].replace(')', ', 0.1)'),
                            borderWidth: 2,
                            tension: 0.35,
                            pointRadius: 4,
                            pointBorderWidth: 2,
                ...
                currentPoints[payload.labels.length] = payload.current; // At 'Current' position
                const afterPoints = Array(labels.length).fill(null);
                afterPoints[payload.labels.length + 1] = payload.after; // At 'After' position

                // Add min stock and stock point datasets
                datasets.push({
                    type: 'line',
                ...
                // Create chart with dynamic datasets
                const chart = new Chart(ctx, {
                    ...
                    options: {
                        ...
                        plugins: {
                            legend: {
                                display: payload.christmasData && payload.christmasData.length > 0,
                                position: 'top',
                                labels: {
                                    usePointStyle: true,
                                    padding: 10,
