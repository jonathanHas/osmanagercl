<x-admin-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6" x-data="invoiceForm()" x-init="init()">
        {{-- Header --}}
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-gray-100">Edit Invoice #{{ $invoice->invoice_number }}</h2>
        ...
        <form method="POST" action="{{ route('invoices.update', $invoice) }}" @submit.prevent="submitForm">
            @csrf
            @method('PUT')
                    ...
                    <div class="bg-gray-800 rounded-lg p-6">
                            ...
                            <div>
                                ...
                                <select name="supplier_id" x-model="supplierId" @change="updateSupplierDetails()"
                                    ...
                                    @foreach($suppliers as $id => $name)
                                        <option value="{{ $id }}" {{ old('supplier_id', $invoice->supplier_id) == $id ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="expense_category" x-model="expenseCategory"
                                    ...
                                    @foreach($categories as $code => $name)
                                        <option value="{{ $code }}" {{ old('expense_category', $invoice->expense_category) == $code ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            <textarea name="notes" rows="2" x-model="notes"
                                      class="w-full bg-gray-700 border-gray-600 text-gray-100 rounded-md">{{ old('notes', $invoice->notes) }}</textarea>
                        </div>
                    </div>
    ...
    <script>
        function invoiceForm() {
            return {
                ...
                supplierId: '{{ old('supplier_id', $invoice->supplier_id) }}',
                supplierName: '{{ old('supplier_name', $invoice->supplier_name) }}',
                dueDate: '{{ old('due_date', $invoice->due_date?->format('Y-m-d')) }}',
                expenseCategory: '{{ old('expense_category', $invoice->expense_category) }}',
                notes: '{{ old('notes', $invoice->notes) }}',
                vatLines: @json($invoiceVatLines ?? []),
                vatRates: {
                    'STANDARD': 0.23,
                    'REDUCED': 0.135,
                    'SECOND_REDUCED': 0.09,
                ...
                init() {
                    // Ensure calculations are correct for loaded data
                    this.vatLines.forEach((line, index) => {
                        this.calculateVatLine(index);
                    });
                    
                    // If no VAT lines exist, create one default line
                    if (this.vatLines.length === 0) {
                        this.addVatLine();
                    }
                }
            }
