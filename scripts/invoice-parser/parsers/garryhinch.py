...
def parse_invoice(text, filename):
    print(f"[DEBUG] Parsing Garryhinch invoice: {filename}", file=sys.stderr)

    try:
        is_credit_note = False
        tax_free = True  # Only 0% VAT observed
        ...
        # === VAT 0% Parsing (Net Total)
        vat_0_match = re.search(r'Total Net Amount\s+€?\s*([0-9]+\.[0-9]{2})', text)
        vat_0 = vat_0_match.group(1) if vat_0_match else "0.00"
        print(f"[DEBUG] VAT 0%: {vat_0}", file=sys.stderr)

        parsed_data = {
            'Filename': filename,
            'Supplier': 'Garryhinch Mushrooms',
