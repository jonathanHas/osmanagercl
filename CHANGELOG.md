...
- **🧾 The customer statement now shows what happened to each payment** (2026-09-05)
  ...
  - **Found and pinned, not fixed:** voiding an invoice leaves its allocation rows in place while `CustomerPayment::getTotalAllocatedAttribute()` counts them with no invoice-void filter, so voiding a fully-paid €100 invoice leaves the customer €100 in credit while the statement, debtors Credit column and payments index all show zero, and the identity gives `0 ≠ -100`. Verified against the live database. The test is committed skipped with the reason; the fix lands separately because both remedies change balance maths outside the statement, and the point of this change is being able to say it moved no numbers
