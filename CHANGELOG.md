...
- **💬 Flash messages on the order pages went nowhere, and always had** (2026-09-09)
  - `redirect()->with('success', ...)` and `back()->with('error', ...)` appeared to do nothing on `/orders/compare` and `/orders/{order}`. The cause is not in the controllers: **`layouts/admin.blade.php` renders no flash block at all**, so every page using `<x-admin-layout>` must render its own, and neither order view did. The messages were flashed, never displayed, and cleared on the next request
  - Found while building the difference-order guards, whose error messages would otherwise have been invisible. It also means `duplicate()`'s "Order duplicated with updated suggestions." has never once been seen since that feature shipped
  - Related: `OrderSession::notes` is written by `update()` but was rendered in **no** order view, so an order's provenance was write-only. Now shown in the order header, which is what lets a difference order explain itself on arrival
  - The gap is app-wide rather than order-specific, so it is written up in known-issues rather than only in the feature doc
  - **Modified**: `resources/views/orders/{compare,show}.blade.php`, `docs/development/known-issues.md`

- **🏷️ Translated labels no longer reprint the whole delivery every time you press Print** (2026-09-09)
  - On `/delivery-legacy/match`, **Print Translated Labels** re-sent labels for products that had already printed in the same delivery, and the button's count never went down. Clearing the printer queue on the machine changed nothing, because two separate things were wrong and neither was the queue
  - The print list was **derived but never consumed**. `getTranslatableScannedProducts()` and `printTranslations()` both rebuilt the set on every request from `SUM(quantity)` over *all* scan rows for the delivery, and after the `lp` call **nothing was persisted** — no `printed_at`, no log, no batch row. Each press re-sent the whole delivery at the current running total: scan 3 → print 3, scan 2 more → print **5**, so 8 labels for 5 units. Completing the delivery didn't clear it either
  - Separately, `lp -h {host}:{port}` submits to the CUPS server at `ZEBRA_PRINTER_HOST` — **the spool is on the printer host, not the web server** — so clearing the local queue was always a no-op, and nothing in the codebase had ever called `cancel`/`lprm`
  - Fixed with a `delivery_label_prints` ledger written **before** the job reaches CUPS. Record-first is the deliberate choice: the dangerous failure is "CUPS accepted it, the response was lost", and print-then-record loses the row and reprints everything next press. A lost or timed-out response can now never duplicate; only a *definitive* refusal (lp ran to completion and returned no request id) stamps `failed_at` and releases the labels back to outstanding. Printing sends `scanned − already_printed`, so a second press sends nothing and the count falls to 0
  - Three compounding defects went with it. **`auto_print` was filtered before the newest-per-code grouping**, so toggling off a product's current translation left an older enabled row matching — the product kept printing, using the **old** ZPL; four duplicated `product_code`s were live, and a data migration reconciled them. **`save()` omitted `auto_print` entirely**, so a new translation fell back to the column default (`true`), silently re-enabling a product you had switched off. And success was inferred from `str_contains($output, 'request id')` while `shell_exec` ran synchronously in the request, so a slow printer timed out PHP *after* CUPS had accepted the job — the user saw "Print failed", `.finally()` re-armed the button, and the obvious next click sent an identical second job. Now each submit carries an idempotency key reused across retries, a `Cache::lock` serialises presses per delivery, a unique index on `(idempotency_key, barcode)` catches the race, and a timeout offers **Refresh** rather than a Retry that could duplicate
  - Also folded the five copy-pasted `lp` blocks into one `ZebraPrintService` with escaped arguments and a `timeout` guard. That was not tidying: `LabelAreaController::printZpl()` **hardcoded `10.42.1.71`** and silently ignored `ZEBRA_PRINTER_HOST`, which only consolidation could fix, and the `lpstat`/`cancel` needed for the new **Printer Queue** card on `/labels/zebra` would otherwise have become a sixth copy of the host string. The pre-existing `lpstat` diagnostic mode queried the *local* daemon with no `-h`, which is why the app had never shown the real spool. `.env.example` had no `ZEBRA_*` keys at all and now does
  - `^PQ{n},0,1,Y` became `,0,0,Y` — the third parameter is *replicates of each serial number* and these labels carry no `^SN`, so a non-zero value is meaningless and doubles output on some GX firmware. **This one is unverified on hardware**: print a translation with `copies = 2`, and 4 labels means it was a second, independent duplication source on top of the main bug
  - 29 new tests, including "second press prints nothing and never reaches the printer" and "scanning more units prints only the delta". Full suite unchanged at 21 pre-existing failures, none of them in this area
  - **Modified**: `app/Http/Controllers/DeliveryLegacyController.php`, `app/Http/Controllers/{LabelTranslationController,LabelAreaController,ZebraLabelController,VoucherController}.php`, `app/Services/{ZebraPrintService,ZebraPrintResult}.php` (new), `app/Models/DeliveryLabelPrint.php` (new), `app/Models/{ProductTranslation,ZebraLabel}.php`, two `2026_09_09_*` migrations (new), `resources/views/delivery-legacy/match.blade.php`, `resources/views/labels/zebra.blade.php`, `resources/views/zebra-labels/create.blade.php`, `routes/web.php`, `config/services.php`, `.env.example`, `tests/Feature/{DeliveryTranslatedLabelPrintingTest,LabelTranslationSaveTest}.php` (new), `tests/Unit/{ZebraPrintServiceTest,ZebraLabelSetZplQuantityTest}.php` (new), `tests/Concerns/AliasesMysqlConnection.php` (new), `docs/features/label-translation-system.md`, `docs/development/known-issues.md`, `docs/FEATURES_INDEX.md`

- **📄 A PDF holding several invoices no longer imports as its last page alone** (2026-09-07)
  - Coolnagrower send a statement page plus several invoices in one PDF, and `coolnagrower.py` correctly returns one record per page — but the loop in `invoice_parser_laravel.py` reassigned `response['data']` and `response['confidence']` on **every pass**, so only the last record survived. Worse, `has_anomalies` was rebuilt per record, so a clean last record **reset the confidence to 0.85** while an earlier record's warning stayed in the list. Invoice 8777 (2025-09-30) is the archived case: four records worth **€591.00 imported as €187.00**, at full confidence, auto-created
  - The formatting body is now a `format_parsed_invoice()` function and the loop **accumulates**. A file with more than one record keeps the rest under `data['additional_invoices']`, reports the combined total as a warning, and is forced to 0.50 so it goes to review. A file can only ever become one invoice downstream, so it has to be split by hand first (`pdf_splitter.py`, via the bulk-upload split route) — review is the correct destination, not a workaround
  - The extraction was verified to change nothing else: **22 synthetic cases covering every branch** of the moved code produce byte-identical `data`, `confidence` and `warnings` against the previous implementation, and a replay of the whole archive — **945 invoices** — changed exactly the four records these two fixes target
  - Note this is the first instance of the "wrong number at full confidence with nothing flagged" family found in the **dispatcher** rather than in a parser; a parser-by-parser audit would not have found it
  - **Modified**: `scripts/invoice-parser/invoice_parser_laravel.py`, `scripts/invoice-parser/tests/test_invoice_parser_laravel.py` (new), `docs/features/invoice-parser-integration.md`, `docs/features/invoice-parser-phase2-implementation.md`, `docs/development/known-issues.md`

- **🥬 The Menton's parser no longer invents VAT or reads totals out of OCR noise** (2026-09-07)
  - Two silent faults in `parsers/mentons.py`. It divided the total by 1.23 and booked the result as **standard-rated** unless the text literally said "tax free"/"zero rated"/"vat exempt" — on a supplier whose **577 archived invoices are all zero-rated** (`zero_net` €92,794.09, `standard_vat` €0.00). A legible €200.10 invoice would have claimed **€37.42 of input VAT that was never charged**, at confidence 0.85, auto-created
  - It also took the first digit run *anywhere* in the document as the total, via `€?\s*([\d,]+\.?\d{0,2})`. Menton's photograph handwritten invoices, so that text is Tesseract's reading of handwriting and is mostly noise: invoices 9394, 9445 and 9069 already parsed as **€7.00, €7.00 and €17.00** against stated totals of €232.00, €232.00 and €58.00. Only an unrelated second failure — the date regex also finding nothing — held them at 0.50 and kept them out of auto-creation
  - Everything now lands in the 0% bucket, and a total is accepted **only from an anchored position**: the `qty @ price = total` line the farm writes consistently, or a labelled total. Anything else reports a warning and returns 0.00, so the file goes to review rather than being created from a number found by accident. **A euro sign is not an anchor** — Tesseract reads `€5/2` out of the 2025-04-17 invoice, whose stated total is €58.00 — so a bare euro-signed amount is deliberately refused
  - The three affected invoices are committed as test fixtures, illegible as they are, because that illegibility is the thing being guarded against
  - **Modified**: `scripts/invoice-parser/parsers/mentons.py`, `scripts/invoice-parser/tests/test_mentons.py` and `tests/fixtures/mentons/` (new), `docs/features/invoice-parser-integration.md`, `docs/features/invoice-parser-phase2-implementation.md`, `docs/development/known-issues.md`

- **💸 Voiding an invoice no longer swallows the payment against it** (2026-09-07)
  - `CustomerInvoiceService::void()` deliberately leaves allocation rows in place so `unvoid()` is a clean round trip — but `CustomerPayment::getTotalAllocatedAttribute()` counted them with **no invoice-void filter**. So voiding a fully-paid €100 invoice left the customer €100 in credit while the statement, the aged-debtors Credit column and the payments index all reported **zero**, and the statement's documented identity `aged total − unallocated credit == closing balance` gave `0 ≠ -100`. Confirmed against the live database before and after
  - Fixed by making `CustomerPayment::allocations()` exclude rows against voided invoices — **the exact mirror of `CustomerInvoice::allocations()`**, which has always excluded rows belonging to voided payments. Chosen over deleting the rows on void because it destroys no data, keeps `unvoid()` the lossless round trip its docblock promises (verified: void frees the credit, unvoid restores the allocation), and reuses a pattern already established in this model pair rather than inventing a second convention
  - A new `allAllocations()` covers the cases that need every row regardless of the invoice's state: the payment page, so a freed row is **shown and explained** rather than silently vanishing — it is marked `invoice voided`, struck through, and totalled as *Returned to credit by voided invoices* via a new `voided_allocated` accessor, so the allocation table still adds up to its own footer — and `reallocate()`, which must actually clear a stale row rather than merely stop counting it
  - The two raw-SQL paths that bypass the relation, `scopeWithUnallocatedOver()` and `unappliedCreditTotals()`, carry the same filter by hand; a test asserts all three agree, because a drift between them would make the payments-index filter and the debtors Credit column disagree with the payment's own figure
  - **Modified**: `app/Models/CustomerPayment.php`, `app/Services/CustomerPaymentService.php`, `app/Http/Controllers/CustomerPaymentController.php`, `resources/views/customer-payments/show.blade.php`, `tests/Feature/{CustomerPaymentAllocationTest,CustomerStatementTest}.php`, `docs/features/customer-statements.md`

- **📧 Every emailed statement failed on an undefined variable** (2026-09-05)
  - Queuing a statement email put the job straight into `failed_jobs` with `Undefined variable $customer` from `emails/customer-statement.blade.php`, while the identical context rendered the screen, print and PDF surfaces without complaint. Three real send attempts failed this way before it was caught
  - A Mailable passes only its **public properties** to its view. `CustomerStatementMail` holds one, `public array $ctx`, so the blade received `$ctx` — not the `$customer`, `$aging`, `$open_invoices` keys it actually reads. Fixed with `Content(view: ..., with: $this->ctx)`
  - **`Mail::fake()` never renders the message**, which is precisely why the test suite missed this: `assertQueued` passes over a view that cannot compile. The new test renders the Mailable for real and asserts the customer name, the balance and the PDF attachment — verified to fail with the exact production error when the fix is reverted, and to pass with it
  - Also confirmed the mailable survives the database queue: a serialize/unserialize round trip still renders and still produces a valid 22 KB PDF attachment. Worth noting that `->send()` on a `ShouldQueue` mailable *queues* rather than sends — use `->sendNow()` to build one synchronously against the `array` transport when debugging
  - **Modified**: `app/Mail/CustomerStatementMail.php`, `tests/Feature/CustomerStatementTest.php`, `docs/features/customer-statements.md`, `docs/development/known-issues.md`

### Added

- **🧾 Kitchen orders can be built, logged and exported per supplier** (2026-09-16)
  - The kitchen products list could only copy supplier codes one at a time; the actual order was assembled by hand elsewhere and nothing recorded what was sent. `/kitchen/orders/create` now shows one supplier's kitchen products (defaulting to the supplier with the most, Udea on live data) with images, supplier code, case size and the last three orders beside each, a cases quantity per row, a sticky footer with live totals and a **Confirm Order** button. Confirming logs the order and lands on a page with **Download CSV**; `/kitchen/orders` is the history with a supplier filter and CSV re-download
  - **Quantities are cases, not units.** `supplier_link.CaseUnits` already matches the supplier's case size and single-unit items carry 1; empty or 0 is normalised to 1 in the service, never in the view. The CSV is `Quantity,Supplier Code,Product Name,Case Size`, one row per line with a quantity above 0, sorted by name. Written by a small quote-only-when-needed writer rather than `fputcsv`, which quotes any field containing a space and would have turned the header into `Quantity,"Supplier Code",…`
  - **The standing weekly order is a pre-fill only.** `/kitchen/standing-order` stores a case quantity per product (keyed by POS product id, so it survives a product being removed from and re-added to the kitchen list); those quantities are filled in with a "standing" badge every time the create page opens. Nothing is sent, emailed or logged automatically — every order is confirmed by a person
  - **Order lines are server-side snapshots.** Supplier code, product name and case size are copied from POS data at confirm time; posted names and codes are never trusted, and posted ids that are not kitchen products of the chosen supplier are ignored. A CSV re-downloaded months later is byte-identical even if POS data has changed. Three new Laravel-side tables; nothing is written to the POS database and the shop's `order_sessions` system is not involved
  - Images are resolved once per product in the service (POS blob via `products.image`, else the supplier CDN) so the page makes four POS queries for 74 products rather than one per row. Where a barcode has several `supplier_link` rows, the link for the selected supplier is pinned before image resolution
  - Tests cover route precedence ahead of the `/{recipe}` wildcard, the create page (pre-fill, badges, supplier switch, bad supplier redirect), confirm (snapshots, totals, all-zero rejection, validation), CSV (exact header, quoting, zero lines omitted, sort), standing order save/clear, and last-3 history isolation per supplier
  - **Modified**: `app/Models/{KitchenOrder,KitchenOrderItem,KitchenStandingOrderItem}.php` (new), `app/Services/KitchenOrderService.php` (new), `app/Http/Controllers/KitchenOrderController.php` (new), `app/Http/Requests/{StoreKitchenOrderRequest,UpdateKitchenStandingOrderRequest}.php` (new), three `2026_09_16_*` migrations (new), `resources/views/kitchen/orders/{create,show,index,standing}.blade.php` (new), `resources/views/kitchen/orders/partials/{product-row,qty-scripts}.blade.php` (new), `resources/views/kitchen/products/index.blade.php`, `resources/views/layouts/admin.blade.php`, `routes/web.php`, `tests/Feature/{KitchenOrderRoutesTest,KitchenOrderCreatePageTest,KitchenOrderStoreTest,KitchenOrderCsvTest,KitchenStandingOrderTest,KitchenOrderHistoryTest}.php` (new), `tests/Concerns/CreatesKitchenOrderPosTables.php` (new), `docs/features/kitchen-orders.md` (new), `docs/features/kitchen-products.md`, `docs/FEATURES_INDEX.md`, `CHANGELOG.md`

- **📋 Customer requests have moved out of the spreadsheet and onto a board that nags** (2026-09-10)
  - Customers asking us to hold a stocked product or source a new one were logged in a spreadsheet that nobody re-read: nothing flagged the item when its delivery arrived or on the day the customer wanted it, and nothing recorded who took the request. `/customer-requests` is now a board that **renders without a login** so the shop-floor tablet can show it all day, with due today / overdue requests on top and a 5-minute auto-refresh for guests only
  - **Every write needs a login** and the new `customer-requests.manage` permission (seeded to employees, so managers inherit it). The public GET sits outside the `auth` route group and everything else inside it, so a guest tapping a write URL lands on the login page rather than a bare 403. The board needed its own `<x-board-layout>` because `layouts/admin.blade.php` calls `auth()->user()->can()` unguarded in the sidebar and fatals logged out
  - A request is one customer (free-text name and phone — deliberately **not** linked to the accounts `customers` table, which is for invoicing) plus a wanted-by date and **one or more lines**, each a stocked POS product picked by scan or typeahead (name snapshotted, `product_code` kept with no FK across databases) or free text to source. Each line has its own status: `pending → ordered → put_aside → collected`, plus `not_available` / `cancelled`, with a small set of undo moves for mis-taps on the tablet. Every move is stamped `status_changed_by/at` and appended to `customer_request_status_logs`; the request's `closed_at` is recomputed from its lines after every change, so "open" is a `whereNull` rather than a `whereHas` and adding a line to a closed request reopens it
  - Three reminders, none of them email: the board itself; a dashboard banner cloned from the Amazon-pending one counting requests due and items put aside awaiting collection; and on `/delivery-legacy/match` a pink **Put aside for &lt;customer&gt; × qty** badge after every product name that is on a `pending` / `ordered` line, a summary card above the filters, and a scanner prompt with a one-tap **Mark put aside** that calls the JSON status endpoint. The lookup is a separate Eloquent query on the app database keyed by barcode — the three raw POS queries on that page are untouched because the two databases cannot be joined. Nothing changes status automatically on scan or on completing a delivery; staff confirm the item is physically set aside
  - The typeahead reuses `Product::search()` (name / barcode / reference / supplier code) rather than the till product browser, which is limited to till-visible products. Enter commits the highlighted match, so a barcode scanner adds a product in one go
  - One MySQL gotcha on the way: with the conventional table name `customer_request_item_status_logs` the auto-generated FK name runs to 66 characters, over MySQL's 64-character identifier limit, and the migration failed half-applied (MySQL DDL is not transactional). The table is `customer_request_status_logs` instead, which keeps the generated name at 61 and needs no special-casing
  - 28 new tests, including the legacy match page rendered end-to-end in sqlite (every table its raw queries touch is built in `setUp()`) and the scan endpoint carrying `customerRequests`. Full suite unchanged at 21 pre-existing failures
  - **Added**: `app/Models/{CustomerRequest,CustomerRequestItem,CustomerRequestItemStatusLog}.php`, `app/Services/CustomerRequestService.php`, `app/Http/Controllers/CustomerRequestController.php`, `app/Http/Requests/{CustomerRequestRequest,UpdateCustomerRequestItemStatusRequest}.php`, `app/View/Components/BoardLayout.php`, `resources/views/layouts/board.blade.php`, `resources/views/customer-requests/**`, `resources/views/delivery-legacy/partials/customer-request-badge.blade.php`, three `2026_09_10_*` migrations, `database/factories/{CustomerRequest,CustomerRequestItem}Factory.php`, `tests/Feature/{CustomerRequestTest,CustomerRequestDashboardTest,CustomerRequestDeliveryFlagTest}.php`, `tests/Unit/CustomerRequestItemTransitionsTest.php`, `docs/features/customer-requests.md`
  - **Modified**: `routes/web.php`, `app/Http/Controllers/DeliveryLegacyController.php`, `resources/views/delivery-legacy/match.blade.php`, `resources/views/dashboard.blade.php`, `resources/views/layouts/admin.blade.php`, `database/seeders/RolesAndPermissionsSeeder.php`, `docs/FEATURES_INDEX.md`, `CLAUDE.md`

- **🧮 An oversized order can now be trimmed and the remainder ordered in one click** (2026-09-09)
  - The order comparison page (`/orders/compare`) existed to answer "what does the smaller order do without?", but was read-only — acting on the answer meant reading a table on one screen and re-keying dozens of products into the search-add box on another. It now offers **Order the difference**: a draft holding, per product, what one order has over the other. Both directions, since the page is symmetric
  - Quantities are the shortfall, `from.final_quantity - to.final_quantity` where positive, with products the other order does not cover coming across in full. **Case products round up to whole cases** — a 25-unit gap on a 12-pack is 3 cases (36 units), which overshoots the raw gap deliberately, because part of a case cannot be bought. Capping it would write a non-case-aligned quantity into a brand-new order and break the invariant every other write path maintains; the raw figure is kept in `context_data.derived_from`
  - **Case size is taken from the source order's item, not recomputed.** `case_units` is a per-session snapshot that drifts between generations while units are the source of truth — the same reasoning the page's existing `caseUnitsDiffer` flag rests on. Worth knowing: `from` at 24 units/12-pack against `to` at 10 units/6-pack is a 14-unit gap, i.e. 2 cases, i.e. **the whole of the source quantity**. For coarse case sizes a difference order can approach the size of the order it came from
  - Item cost, priority and the entire `context_data` sales/stock snapshot are **copied rather than recalculated** — that snapshot is what the buyer was looking at when the shortfall arose. `auto_approved` and `added_via_search` are reset, since nobody has approved the new quantity and the rows are the order's own content
  - The new session copies the supplier and sales window, **re-anchors** `coverage_ends_on` to the delivery date chosen on the form (as `duplicate()` does — the source order's window may already have closed), drops `coverage_overrides` (their dates belong to the source order's cycle) and forces `christmas_comparison_enabled` off, since `show()` would otherwise divert straight off the page the user was just sent to. The delivery date is set on the form because `OrderController::update()` accepts only `notes` — an order's date **cannot be changed after creation**, so getting it wrong would be unfixable
  - The button's own count runs through the same code as the write, so a preview promising 112 units cannot produce an order of 120. Deriving it from the already-computed comparison collections would have been free but would have shown the *raw* delta against *case-rounded* quantities
  - Blocked when the two orders are for different suppliers — an `OrderSession` belongs to exactly one, so the difference has no valid `supplier_id`. **Caveat**: regenerating a difference order calls `regenerateOrderSession()`, which deletes all items and rebuilds from live suggestions, discarding the difference; nothing in the schema marks an order as derived, so the `notes` string warns about it instead
  - Throughout, **"ordered" means `final_quantity > 0`** — a session holds a candidate row for every supplier product, most at quantity zero (session #1: 1,504 rows, 353 actual orders). The predicate now lives in one place, `OrderService::orderedItemsByProduct()`, shared by the comparison and the difference builder. A side effect: a difference order's Items count on `/orders` is the true product count, so it reads ~37 where its neighbours read ~1,400
  - Verified end-to-end against live sessions #300/#301 inside a rolled-back transaction (37 products, 112 units, €254.80, correct CSV export). 16 new tests: the arithmetic runs against **unsaved** models deliberately — the `decimal:3` casts return strings there exactly as on a persisted row, so the string-vs-float hazard is exercised for real, with no database or POS scaffolding needed
  - **Added**: `tests/Unit/OrderDifferenceTest.php`, `tests/Feature/OrderDifferenceCreationTest.php`, `docs/features/order-management/order-comparison.md`
  - **Modified**: `app/Services/OrderService.php`, `app/Http/Controllers/OrderController.php`, `routes/web.php`, `resources/views/orders/{compare,show}.blade.php`, `docs/features/order-management/order-generation.md`, `docs/FEATURES_INDEX.md`, `docs/README.md`, `docs/development/known-issues.md`

- **📦 Udea pallet volumes: capture the data, and show pallet fill while building an order** (2026-09-09)
  - Udea publishes how much pallet space each product occupies, but **only on the logged-in basket page** (`/orders/cart`), and only for products currently in the basket. Verified this session that `/products/product/{slug}`, `/search/?qry={code}` and every script the basket loads carry none of it, and that there is no endpoint serving it — the calculator in `js/orders/order.js` reads the values straight out of the DOM. Capturing it therefore means putting products in a basket and reading them back, which is why this ships as a sync tool rather than a scraper
  - **New admin page, System Tools → Udea Pallet Volumes** (`role:admin`): shows coverage, then a **Sync from basket** button. The workflow is empty the basket → bulk-upload the range through Udea's own Excel importer → sync. The page is **read-only against Udea** — it never adds, changes or removes anything there; all basket manipulation stays manual and deliberate. `php artisan udea:sync-pallet-volumes [--dry-run]` does the same from the CLI
  - **The sync cannot run in-request and doesn't**: a basket holding a full range is ~15MB of HTML while PHP-FPM here is capped at `max_execution_time = 30` / `memory_limit = 128M`. The controller shells out to the artisan command and streams the output, so the work runs under CLI PHP where those limits are lifted — following the existing `OSAccountsImportController::streamCommand()` pattern, which also avoids depending on a queue worker being alive on production
  - **Storage is three columns on `udea_product_cards`**, not `PRODUCTS` and not a new table: it is already "durable cache of scraped per-product Udea facts", is a strict 1:1 on `supplier_code`, and that is the join key back to `supplier_link.SupplierCode`. A generic `supplier_pallet_volumes` table was rejected — no other supplier publishes volumes, and one that did would not share Udea's 250/360 model, so it would cost a join on every order page to buy speculative generality
  - **`pallet_sve` and `pallet_unit_volume` are `DECIMAL(10,4)`, not integers.** `sve` looks like a case size and matches POS `CaseUnits` for 252 of 260 products checked — but weight-priced goods break it: `45006` Kipfilet is **0.22**, Roquefort 1.44, Gruyère 2.5, Geitenkaas 4.5. An integer column would have stored 0.22 as **0** and silently zeroed those lines. They are cast to `float` rather than `decimal:4`, which returns strings and breaks the arithmetic. A test asserts 0.22 and 2.5 survive both the parse and the database round trip
  - **`pallet_scraped_at` is deliberately separate from `scraped_at`**, which drives the 30-day price-tier staleness gate in `UdeaScrapingService::debugProductCard()`. Writing it here would have made the price cache look fresh when it wasn't, suppressing legitimate re-scrapes for thousands of products. Verified safe both ways: `persistCard()` uses an explicit attribute list so prices survive a pallet sync, and pallet-only rows keep `scraped_at` null so they don't suppress price scrapes
  - **Running pallet total on the order review page** (`/orders/{order}`, Udea suppliers only): `−`/`+` steppers for Europallets and blockpallets, a fill bar and a live percentage that recalculates as quantities change, mirroring Udea's own calculator including `totalPalletVolume = numEuro × 250 + numBlock × 360`. Green → amber past 90% → red over 100%; the pallet selection persists per-browser. Lines with a quantity but no pallet data are counted and flagged, so an understated total is always **visible as such** rather than quietly wrong. `/orders/create` deliberately has none — it is the generation form and has no line items
  - **Two details reproduce Udea's displayed figure and are easy to miss.** `sve` is a multiplier, not a case count — omitting it turns a 470.66 basket into 141.04 — and each line's percentage is rounded to 2dp **before** summing, where rounding once at the end gives 188.26% against their displayed 188.24%. Both are pinned by a golden-master test built from a real 298-line basket (719 units, €5,675.09) whose figures were confirmed against the live site: volume **470.6612**, **188.24%** of a Europallet, **130.80%** of a blockpallet. It is price-independent, so it catches quantity drift that money totals would mask
  - **Bug found and fixed during verification**: the first parser anchored on the `product-pallet-data` span, but the `Productnummer` label sits *before* it within the row — so every product was paired with the **next** row's code. The volume total was right, which is exactly why it would have survived a casual check. Now split on row boundaries; all 298 rows match the verified snapshot
  - Note the sync's **"Updated" counter reports whether the row existed, not whether pallet data did** — the price scraper creates rows on the same `supplier_code`, so a previously price-scraped product counts as an update with empty pallet columns. Documented, since it reads as a duplicate and isn't
  - **New**: `app/Services/UdeaPalletVolumeService.php`, `app/Console/Commands/SyncUdeaPalletVolumes.php`, `app/Http/Controllers/UdeaPalletVolumeController.php`, `resources/views/tools/udea-pallet-volumes.blade.php`, `resources/views/components/udea-pallet-summary.blade.php`, `database/migrations/2026_09_09_000100_add_pallet_volume_to_udea_product_cards_table.php`, `tests/Unit/UdeaPalletVolumeServiceTest.php`, `tests/Feature/UdeaPalletVolumeSyncTest.php`, `tests/Fixtures/udea-cart-298-lines.json`, `docs/features/udea-pallet-volumes.md`
  - **Modified**: `app/Models/UdeaProductCard.php`, `config/suppliers.php`, `routes/web.php`, `resources/views/layouts/admin.blade.php`, `resources/views/orders/show.blade.php`, `resources/views/orders/show-christmas.blade.php`, `resources/views/orders/partials/review-table.blade.php`, `resources/views/orders/partials/review-table-christmas.blade.php`, `docs/FEATURES_INDEX.md`
  - **Known limitations**: the A2 and Grid order layouts carry their own quantity markup rather than sharing `review-table.blade.php`, so they have no pallet bar; and `sve` vs POS `CaseUnits` disagree for a small number of weight-priced products, so an order is worth spot-checking against Udea's calculator once coverage is complete

- **🧾 The customer statement now shows what happened to each payment** (2026-09-05)
  - The statement was never updated when payment↔invoice matching shipped, and it lagged that work on every surface. A payment was one prose string — `Payment — Bank transfer (applied to INV-001, INV-002)` — so a €500 payment split €300/€200 showed **neither amount**, and a payment applied to *nothing* got no marker at all, reading identically to a fully matched one. The views rendered only that string while the payment object, with its allocations already eager-loaded, sat unused in the context
  - Payment events now carry structured `allocations` / `allocated` / `unapplied` / `allocation_summary`, rendered as a **sub-line under the description** — linked per-invoice amounts on screen, the one-line form on paper. Not a sixth column: it would take width from the only column that wraps on a 150mm PDF, be blank on every invoice row, and wrap anyway on a three-way split — and an in-cell sub-line is protected by `page-break-inside: avoid` where a `colspan` detail row is not. `_pdf.blade.php` was missing that rule entirely, so it has been added to match the print view now that ledger rows are multi-line
  - **New "Payments on account" section** listing unapplied payments with dates, methods and references. The single note it replaces could not do this job: `unallocated_credit` spans every payment up to the as-at date while the ledger only covers the selected range, so on a February statement a credit received in January was a figure with **no supporting row anywhere on the document** — uncheckable by the customer and by us. Its rows sum to `unallocated_credit` exactly, and a test asserts that
  - **Fixed: the screen's credit note vanished exactly when it mattered.** It was nested inside `@if (! empty($open_invoices))`, so a customer holding pure on-account credit with no open invoices saw nothing at all
  - **Fixed: the print/PDF wording was factually wrong** on a customer-facing document — *"Includes €X received but not yet applied"*, when nothing above it included the credit (the aged total is by definition *before* it). Both surfaces now carry one canonical sentence — "€X outstanding on invoices, less €Y received on account, leaves a balance of €Z" — which is the documented identity written for a customer. The document now **shows its own proof**: a future change that breaks the identity produces a visibly wrong sentence instead of a silently wrong number
  - **The emailed statement**, the surface customers actually read, showed three columns with no Paid figure and no mention of credit — a part-paid invoice showed only the residual with no acknowledgement that money had arrived, which is the likeliest source of "you didn't credit my payment" replies. It now has a Paid column (only when something is part-paid, so an all-unpaid statement keeps its three columns) and, when credit exists, spells out `Invoices outstanding − Payments received on account = Balance` in the headline box, since those two figures previously disagreed with nothing explaining why
  - The screen gained a `part-paid` tag, the payments that paid each open invoice, and links into the matching flows (*match →* on an unapplied payment, *apply credit →* on an outstanding invoice). Per-invoice payment history is loaded in `build()`, **not** `openInvoices()` — `debtors()` calls that once per customer and would pay a query per debtor row; a query-count test pins the distinction
  - Found while mapping the surfaces: voiding an invoice swallowed its payment. Fixed separately, below
  - **Modified**: `app/Services/CustomerStatementService.php`, `resources/views/customer-statements/{show,_body,print,_pdf}.blade.php`, `resources/views/emails/customer-statement.blade.php`, `tests/Feature/CustomerStatementTest.php`, `docs/features/customer-statements.md`

- **🔗 Customer payments can now be matched to invoices after they are recorded** (2026-09-05)
  - Allocating a payment to invoices was a **one-shot, create-time-only decision**. `Route::resource('customer-payments')->except(['edit','update'])` meant that once a payment was saved, its allocations were frozen — a payment recorded with no allocations stayed on-account credit forever, and the only remedy was voiding it and re-recording, which destroys the audit record. There was no way to mark an invoice settled from the invoice side, and unapplied credit was invisible outside a single payment's detail page. On the live dataset this had already left **€371.67 across 2 payments unmatched**, one of them (€265.81) an exact match for an open invoice
  - **Edit allocations** on any payment (`customer-payments.allocations.edit`) replaces its allocation set wholesale, reusing the same allocation table as the record-payment form. A one-click **Match oldest first** wires up `CustomerPaymentService::autoAllocate()`, which existed but was connected to nothing. The payment itself stays immutable — amount, date, method and till are fixed once banked, which is why this is deliberately *not* `customer-payments.edit`/`update` — and re-matching is stamped with `last_matched_at` / `last_matched_by`, mirroring `customer_invoices`' existing edit-audit columns
  - **Each invoice row has a checkbox** — tick to settle it, untick to clear it — so a payment that matches an invoice no longer has to have its figure retyped into the row. An invoice whose outstanding amount equals the payment exactly is badged *exact match*. On the edit page the banked amount is the ceiling, so a tick applies whatever is left and the checkbox disables when nothing is; on the record-payment form, where the amount is still an input, a tick that doesn't fit grows the amount, matching what *Auto-allocate* already did
  - **Apply existing credit** from an outstanding invoice (`customer-invoices.apply-credit`) draws on the customer's unapplied payments oldest-first, topping up an existing allocation row rather than adding a second one for the same pair
  - **Fixed silent money loss.** `recordPayment()` checked only that allocations didn't exceed the *payment* — nothing stopped allocating €500 to a €50 invoice, allocating to a **void** invoice, or submitting two rows for the same invoice that each passed a per-row check but jointly busted it. Because `outstanding_amount` clamps at 0, the excess vanished from every balance report and broke the identity the statement depends on (`aged total − unallocated credit == closing balance`). All four holes are now closed by one shared validator every write path passes through
  - **Headroom is computed by a grouped join, not the `outstanding_amount` accessor.** An invoice paid in full by payment P reads as €0 outstanding, so editing P's own allocations has to add its rows back or the invoice P already pays would be capped at zero and drop out of the form entirely. The same join reproduces the accessor's void-payment filter. Two queries regardless of invoice count; the result is deliberately not clamped at 0 so legacy over-allocation stays visible
  - **Concurrency**: two transactions each reading €50 of headroom and each writing €50 would put €100 on a €50 invoice. Nothing portably gap-locks allocation rows, so the invoice row is the mutex — every write locks `customer_invoices` before summing. Lock order is fixed everywhere (payment row first, then invoice rows ascending by id) so the paths cannot deadlock
  - **Unapplied credit is now visible** where it matters: a header stat and an *Unmatched only* / *Partly matched* filter on the payments index (filtered in SQL, before pagination, via a new `scopeWithUnallocatedOver` — inlining its threshold with `%F` for the same reason `Customer::scopeWithBalanceOver` does, since SQLite binds a PHP float as text), a Credit column on the aged debtors report and its CSV, and a line on the customer page. This also *explains* a debtors row whose aged total exceeds its balance, which previously read as a bug
  - The allocation table is now one shared component rather than ~130 duplicated lines of Blade and Alpine, with its state extracted to `customerAllocationCore()` that each page spreads into its own component
  - **Added**: `app/Http/Controllers/CustomerPaymentAllocationController.php`, `app/Http/Requests/CustomerPaymentAllocationRequest.php`, `app/Http/Requests/CustomerPaymentRequest.php`, `database/migrations/2026_09_05_120000_add_match_audit_to_customer_payments.php`, `database/factories/CustomerInvoiceFactory.php`, `database/factories/CustomerPaymentFactory.php`, `resources/views/components/customer-payments/allocation-table.blade.php`, `resources/views/customer-payments/_allocation-script.blade.php`, `resources/views/customer-payments/allocations.blade.php`, `resources/views/customer-invoices/apply-credit.blade.php`, `tests/Feature/CustomerPaymentAllocationTest.php`
  - **Modified**: `app/Services/CustomerPaymentService.php`, `app/Services/CustomerStatementService.php`, `app/Models/CustomerPayment.php`, `app/Http/Controllers/{CustomerPayment,CustomerInvoice,Customer,CustomerDebtors}Controller.php`, `routes/web.php`, `resources/views/customer-payments/{create,index,show}.blade.php`, `resources/views/customer-invoices/show.blade.php`, `resources/views/customers/{show,debtors}.blade.php`, `docs/features/customer-statements.md`

- **🧾 Customer statements: printable, aged, emailable, with an aged-debtors report** (2026-09-05)
  - A statement existed but could not be *printed*: the only screen view is dark-themed (`bg-gray-800`/`gray-900`) with no `@media print` rules, so browser-printing produced a black page and PDF download was the sole route to paper. A dedicated light-themed A4 `print.blade.php` now carries `@page` sizing, a repeating `thead` and `page-break-inside: avoid`, kept **separate from the Dompdf view** because the two renderers disagree on fixed footers and `@page` handling
  - It also could not tell a customer *what to pay*: balance-forward only, with no aging and no open-item list, even though `outstanding_amount` and `paymentStatus()` were already written. Both now appear on screen, print and PDF
  - **`customer_invoices.due_date` was null on every invoice** — the field is on the form and validated, but nobody filled it in — so aging keys off an *effective* due date: `due_date ?? issue_date + customer.payment_terms_days` (new column, default 30). Derived dates are marked `*` on screen so they are not mistaken for agreed terms, and `CustomerInvoiceService::createDraft()` now fills `due_date` from those terms so the fallback stops mattering over time
  - **A payment with no allocation reduces no invoice**, so the aged total could overstate the debt and disagree with the balance. `unallocated_credit` is reported separately and the identity `aged total − unallocated credit == closing balance` is asserted in the suite
  - **Emailing**: opt-in per customer (`send_statements`, default false), queued Mailable with the PDF attached, `statement_last_sent_at` tracking, plus `customers:send-statements` (with `--dry-run`) scheduled monthly on the 1st
  - **Aged debtors**: balance column and *owing only* filter on the customer list, a bucketed report with totals and CSV export. The old outstanding-balances widget loaded every customer with invoices and allocations into PHP and then fired two more queries per row via the `balance` accessor; both pages now aggregate in SQL, and a test pins the query count flat from 3 customers to 20
  - Filtering by balance uses correlated subqueries in `WHERE`, not `HAVING` on a `withSum` alias, because **neither alias form is portable** — MySQL's `ONLY_FULL_GROUP_BY` rejects `GROUP BY` + `HAVING` here and SQLite rejects `HAVING` without `GROUP BY`. The threshold is inlined with `%F` because **SQLite binds a PHP float as text, and text compares greater than every number**, so a bound `> ?` silently matched nothing — it returned zero rows on SQLite while working on MySQL
  - One `CustomerStatementService::build()` feeds every surface, so screen, paper, PDF and email cannot disagree. Branding moved to `App\Support\DocumentBranding` rather than a Blade partial because `@include` has its own variable scope and the extracted `@php` preamble never reached the parent view
  - **Added**: `app/Services/CustomerStatementService.php`, `app/Support/DocumentBranding.php`, `app/Mail/CustomerStatementMail.php`, `app/Jobs/SendCustomerStatementsJob.php`, `app/Console/Commands/SendCustomerStatements.php`, `app/Http/Controllers/CustomerDebtorsController.php`, `resources/views/customer-statements/{print,_body}.blade.php`, `resources/views/customers/debtors.blade.php`, `resources/views/emails/customer-statement.blade.php`, `docs/features/customer-statements.md`
  - **Modified**: `app/Http/Controllers/{CustomerStatementController,CustomerController,CustomerPaymentController}.php`, `app/Models/{Customer,CustomerInvoice}.php`, `app/Services/CustomerInvoiceService.php`, `resources/views/customers/{index,create,show}.blade.php`, `routes/{web,console}.php`

- **📖 The fresh install guide now describes the database setup the project actually uses** (2026-08-11)
  - `docs/development/fresh-install-guide.md` existed but **nothing in the repo linked to it**, and its POS section described a setup that has never been correct: `unicenta` on port 3306 reached through a local `pos_readonly` MySQL user. In reality the POS snapshot runs in a pinned **Docker `mysql:5.7.33` container** published on **port 3307**, and that container holds *two* databases — `unicenta2016` **and** `OSAccounts`. Docker was not mentioned anywhere in the guide and the `osaccounts` connection was undocumented entirely, so a machine built from it came up with no product, sales, or invoice data
  - Rewritten around the fact that **the app needs three databases across two engines**: `osmanagercl` on native MariaDB 10.11 (`:3306`), `unicenta2016` and `OSAccounts` on the MySQL 5.7 container (`:3307`). A summary table now leads the document, because the split is the thing most likely to be got wrong
  - **A `-P 3307` without `-h 127.0.0.1` is silently ignored** — the client falls back to the local Unix socket and connects to MariaDB instead. This is called out prominently and every command in the guide passes both flags. The same confusion has already caused F&V price sync to write to the wrong uniCenta instance (see `docs/troubleshooting/backend-system-issues.md`)
  - **New "Load the data" section**, which the guide previously had no equivalent of — a migrated install is empty and nearly every screen is blank. Covers importing the app database from production (`lilThink2`), POS and OSAccounts snapshots from the till server (`shop`), and rsync of `storage/app` plus the sibling `OSManager/invoice_storage` directory. Records the **`osmanager` → `osmanagercl` rename**: dumping with `--databases` embeds `USE osmanager`, so the import recreates the production name and the dev database stays empty
  - **Seeder ordering is now documented.** `DatabaseSeeder` never calls `RolesAndPermissionsSeeder`, while `AdminUserSeeder` falls back to `'role_id' => null` when the admin role is missing — so a bare `php artisan db:seed` yields an admin account with no permissions
  - Also new: Docker install and the `usermod -aG docker` re-login step, queue/scheduler wiring (`routes/console.php` jobs need a crontab entry that most dev boxes should *not* have), optional HTTPS for the camera barcode scanner, the stale `public/hot` trap, and the dev tooling that lives outside the repo (`mpg123` + notification sound, the gitignored `.claude/settings.local.json`)
  - **Fixed six dead documentation links.** `README.md` (×2), `docs/README.md` (×2), `CONTRIBUTING.md` and `docs/features/label-system.md` all pointed at `docs/development/setup.md`, **a file that has never existed**; they now point at the fresh install guide. The stale `development/testing.md` and `development/coding-standards.md` links were repointed too
  - **Corrected the SQLite claim** in `README.md` and `docs/development/system-requirements.md`. Both described SQLite as the primary database; the project runs MySQL/MariaDB, `config/database.php` sets the `default` fallback to `mysql`, and `database/database.sqlite` is a 0-byte placeholder
  - **Modified**: `docs/development/fresh-install-guide.md`, `docs/README.md`, `README.md`, `CONTRIBUTING.md`, `CLAUDE.md`, `docs/features/label-system.md`, `docs/development/system-requirements.md`, `docs/development/quick-start-guide.md`
...
- **🧾 Split-payment receipts inflated net sales and VAT on every accounting report** (2026-08-11)
  - `JOIN PAYMENTS ON RECEIPTS.ID = PAYMENTS.RECEIPT` fans out: a receipt paid part cash / part card has two `PAYMENTS` rows, so **every ticket line on it was counted twice**. The join exists only to attribute sales to a payment type, but it silently multiplied the amounts it was grouping. Four call sites carried it — the daily import into `sales_accounting_daily`, the VAT returns real-time fallback, the sales accounting report fallback, and both P&L queries
  - Ticket line values are now **apportioned across a receipt's payment types by each payment's share of the receipt total**. Single-payment receipts get a share of 1 and are unchanged, so only split receipts move. Receipts whose payments net to zero (debt settlements) are excluded — they carry no ticket lines
  - The same fan-out **over-deducted gift vouchers**. The paperin adjustment took the *whole receipt* gross for any receipt touching a voucher, so a €20 voucher against a €37.94 sale deducted €37.94, and a €50 voucher against €16.22 of goods deducted €16.22 twice over. Apportionment deducts the voucher's actual share, and caps it at the goods value where the voucher exceeds the sale — the remainder is breakage, never counted as revenue a second time
  - Measured on August 2025: net sales **104,054.23 → 103,531.16** (−523.07 double-count), voucher adjustment **311.99 → 257.22**, giving a VAT-return net of **103,742.24 → 103,273.94**. Roughly 0.45% overstatement, in the direction of **overstating VAT liability**
  - `sales_accounting_daily` and `stock_transfer_daily` were **rebuilt across all 986 days** (2022-12-31 → 2025-09-11) with `sales-accounting:import --force`. **Returns already submitted are unaffected** — they store their own `total_net` / `sales_vat_data` snapshot — so a period filed before this date will no longer reconcile against the live page. A dated note on `/management/vat-returns/{id}` explains the discrepancy
  - The query now lives once, in `SalesAccountingImportService::getApportionedSales()`, so the four call sites cannot drift apart again
  - **Modified**: `app/Services/SalesAccountingImportService.php`, `app/Http/Controllers/Management/VatReturnController.php`, `app/Http/Controllers/Management/SalesAccountingReportController.php`, `app/Http/Controllers/Management/ProfitLossController.php`, `resources/views/management/vat-returns/show.blade.php`

- **📊 Sales review counted internal stock transfers as revenue** (2026-08-11)
  - "Total Revenue" on `/management/sales-review` was **not comparable to VAT returns** and read ~3.1% high for August 2025. It drew from `sales_daily_summary`, built from `STOCKDIARY` with no customer filter, so **stock issued to the Kitchen and Coffee departments counted as sales** (€3,318.00 in August). VAT returns and the sales accounting report have always excluded them
  - The import now sources from `TICKETLINES` instead of `STOCKDIARY` and applies the same `Kitchen`/`Coffee` exclusion. `STOCKDIARY` carries no ticket or customer reference, so the customer link was the only way to identify a transfer; both sources produce **identical revenue** once the filter is removed, confirmed to the cent
  - Transfers are **not hidden** — a new *Internal Transfers* tile shows the value with a per-department split and its own year-on-year change, so the number stays visible rather than vanishing from the page
  - The revenue tile is relabelled **"Total Revenue (ex-VAT)"**. `TICKETLINES.PRICE` is the ex-VAT unit price, so this figure has always been net — the unqualified label invited comparison against gross till totals
  - `sales_daily_summary` was rebuilt for its full range (2024-11-30 → 2025-09-11). Stale rows were deleted first: a product sold *only* to Kitchen/Coffee on a given day has no replacement row, so an update-in-place would have left its old inflated value behind
  - **Note**: the "Transactions" tile still counts *ticket lines*, not receipts, so "Avg Transaction" remains understated. Pre-existing and unchanged here
  - **Added**: `app/Models/StockTransferDaily.php`
  - **Modified**: `app/Services/SalesImportService.php`, `app/Http/Controllers/Management/SalesReviewController.php`, `resources/views/management/sales-review/index.blade.php`

- **🚀 Deploy script chowned production away from the account that deploys it** (2026-08-08)
  - The post-deployment block ran `sudo chown -R www-data:www-data . && sudo chmod -R 755 .` over the whole tree. `jon` is not `www-data`, so the moment that actually ran, **the next rsync could not create its temporary files** — one `mkstemp … Permission denied` line per file, exit 23, and **nothing transferred**. The abort was correct; the tree was the problem
  - It was not only the next deploy. Every step *after* the chown in the same run also executes as `jon` — `composer install`, `config:cache`, `route:cache`, `view:cache`, `migrate --force`, and the `php -r` that writes `storage/app/build-version` — and all of them need to write. The block had only ever looked harmless because passwordless sudo was unavailable in the non-interactive ssh session and it silently did nothing. **v3's `sudo -n` probe made it visible, not newly broken** — and once the chown ran manually, the latent breakage surfaced
  - Distinguishing this from the exit-23 noise fixed on 2026-08-04 matters: `failed to set times` means the contents transferred and only an attribute was refused, so continuing is safe. `mkstemp … Permission denied` means the file **never arrived**. The classifier already treated the second as fatal — the failure was real
  - Replaced with the standard Laravel layout: the tree is owned by the deploying user with group `www-data`, and **only `storage/` and `bootstrap/cache/` are group-writable**, because those are the only paths the framework writes at runtime. The web server merely reads application code. Ownership is now one-time server setup rather than a per-deploy action; the deploy only re-asserts group-write on those two paths, which rsync can clear on newly created files
  - New `check_production_permissions()` runs **before** rsync and aborts with the exact repair commands if the destination is not writable, instead of failing part-way through a transfer. A genuine exit 23 now also explains itself when the cause is `mkstemp`
  - The `public/storage` symlink no longer needs sudo — it needed it only because the chown had handed `public/` to `www-data`
  - **Modified**: `scripts/deployment/deploy/deploy-streamlined3.sh`
