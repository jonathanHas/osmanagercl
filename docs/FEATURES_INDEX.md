...
- **Highest Value Stock Lines** (2026-07-28): top 50 lines by value (excluding cost anomalies) with **days of cover** — units divided by the 90-day sales rate — to distinguish a fast seller from a wrong quantity. Quantities can be corrected inline; changes are recorded in the existing `stock_adjustments` table with a `valuation_high_value` source. Lazy-loaded so the velocity lookup never lands on page load
- **No-Reload Updates**: corrections apply via AJAX (58 ms round trip vs a 1,000 ms / 460 KB page reload), with the Total Stock Value, anomaly count and anomaly subtotal updating live from an exact server-returned delta. Requires JavaScript
