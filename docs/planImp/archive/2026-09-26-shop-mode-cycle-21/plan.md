...
Read `implemented.md` to the end and the diffs of the search service, the request service and controller, the new component, the three views and both test files. Reran `php artisan test`: 15 failed / 653 passed, the identical set (640 + 13). Design block untouched; no stylesheet change; the three `app/` diffs are the new public method and constant, the constructor and row enrichment, and `show()`/`seedItems()`.

**Steps 1–5: pass.** One resolver, reused by the board, the detail page and the edit seed; the query never selects the blob, asserted against the query log; the component falls back to the placeholder on an image error; sourcing lines are byte-for-byte unchanged. The implementer checked the image route's middleware first, walked all four screens with a throwaway request, and fetched the public board with no cookies to show the documented guest behaviour (supplier pictures load, POS photos fall back).

**Deviations:** none. The fixture needing the full `SELECT_COLUMNS` set and the enrichment living in `rowsFrom()` are both right.

**Notes for Planner.**
1. Guests see placeholders for POS-photo products: **stated in scope; decision recorded**: if wanted, a guest-readable thumbnail on the board rather than opening `products.image`.
2. Three other screens hold product codes without pictures (deliveries, order review, label queue): **candidates**, not now.
3. Pre-order titles are wrapped in a `shop-inline`, sourcing titles are not: **noted**.
4. Any test that renders a page reaching the search service needs the full `SELECT_COLUMNS` fixture: **noted in memory**.

**Verdict: ACCEPTED.** Archive to `docs/planImp/archive/2026-09-26-shop-mode-cycle-21/`.
