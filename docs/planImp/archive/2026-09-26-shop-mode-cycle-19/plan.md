...
Read `implemented.md` to the end and the diffs of `rows()`, the harvest script, the view and the tests. Reran `php artisan test`: 15 failed / 636 passed, the identical set (634 + 2). Design block untouched; no route helper in the script; the print URL reaches the module as a Blade-built template.

**Steps 1–5: pass.** The offer appears after a log only when the product has a label, defaults copies to the rounded amount, sends through `zebra-labels.print`, closes on success and stays open with the message on failure; Today rows reprint; the fake-printer test pins `^PQ3` (one job, three copies). The implementer exercised the card on the existing Today row without printing or creating data, as the plan required.

**Deviations:** none. Capturing the amount before `typed` is cleared was necessary and is right.

**Notes for Planner.**
1. Only three of Jon's products have an active label on dev (Spinach_Mossfield, Mixed Salad, Rocket_Mossfield): **told to the owner** so the first real test uses one of them.
2. A small weight rounds to one label, never zero: **accepted**, the office rule with a sensible floor.
3. Failure shown as a toast and on the card: **accepted**, the card line explains why it is still open.
4. Office and Shop share the endpoint, not the UI: **noted**.

**Owner's part of the check:** the printer producing the labels, and the "Couldn't confirm the print job" path with it off.

**Verdict: ACCEPTED.** Archive to `docs/planImp/archive/2026-09-26-shop-mode-cycle-19/`.
