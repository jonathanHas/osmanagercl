...
- **Saves accumulate, within one unit**: `harvest.save-row` *adds* the submitted amount to whatever is already logged for that (date, product), unlike the waste log which replaces. There is a single quantity column, so an entry whose unit differs from the day's existing row is **refused** with 422 and a message naming what is logged — before 2026-09-26 it was accepted, and 5.2 kg followed by 2 units became a row reading "7.2 unit". A product with no row that day still takes either unit. Over-logging, or a genuine change of unit, is corrected by deleting the day's line in history.
...
After a successful harvest log, a product that has an active Zebra label gets a print card (cycle 19): copies default to the amount just logged, rounded — 4.2 kg is four labels, a count maps one to one — with a stepper, a reminder of the label size, and Skip. Each Today row with a label also carries a printer button for a reprint, defaulting to everything logged so far. The print goes to the existing `zebra-labels.print` endpoint (`labels.print`, which employees hold) — no ZPL is handled in Shop code, and the controller rewrites `^PQ` so N copies is one job. A failed print keeps the card open with the printer's message so it can be retried; the harvest entry is saved before the offer appears, so skipping or failing a print never loses the log. Products without a label behave exactly as before.

The rows endpoint returns `logged_unit` — the unit today's row is actually in, or null when nothing is logged — which is not always the product's remembered preference. The Shop screen locks its kg/units switch to it and explains why, so staff meet the refusal above only if another till logged the product first; the screen then re-reads and the lock catches up.
