...
# Target directory for copying processed PDFs to Dropbox
PDF_COPY_TARGET_DIR = "/home/jon/Dropbox/Daily/invoices/Print/independent"
# Target directory for copying processed PDFs for upload
PDF_UPLOAD_TARGET_DIR = "/home/jon/Documents/Invoices_in_limbo"
# Target directory for moving successfully parsed PDFs to limbo
...
class IIHInvoiceParser:
    ...
    def generate_report(self, all_rows: List[Dict], output_base: str):
        ...
        report_lines = []
        report_lines.append("# IIH Invoice Parsing Report (v10)")
...
def main():
    parser = argparse.ArgumentParser(description='IIH Invoice Parser v10 - Dual output (new Laravel + legacy system) with upload directory support')
    parser.add_argument('path', nargs='?', default='.', help='Path to PDF file or directory (default: current directory)')
    parser.add_argument('-o', '--output', default='iih10_output', help='Output filename base (default: iih10_output)')
    ...
    if all_rows:
        ...
        # Copy processed PDFs to upload directory
        print(f"\n📂 Copying PDFs to upload directory: {PDF_UPLOAD_TARGET_DIR}")
        upload_success, upload_failure = copy_pdfs_to_target(source_directory, PDF_UPLOAD_TARGET_DIR)
        if upload_success > 0:
            print(f"✅ Copied {upload_success} PDF(s) to upload directory")
        if upload_failure > 0:
            print(f"❌ Failed to copy {upload_failure} PDF(s) to upload directory")
