...
PDF_COPY_TARGET_DIR: Final[str] = "/home/jon/Dropbox/Daily/invoices/Print/Udea"
...
def clear_target_directory_pdfs(target_dir: str) -> int:
    """
    Delete all PDF files in the target directory.
    ...
    for pdf_file in existing_pdfs:
        try:
            os.remove(pdf_file)
            deleted_count += 1
            logging.debug(f"Deleted existing PDF: {os.path.basename(pdf_file)}")
    ...
    for fname in os.listdir(pdf_folder):
        if fname.lower().endswith(".pdf"):
            ...
            try:
                ...
                success_count += 1
                logging.debug(f"Copied PDF: {fname} -> {target_dir}")
            except Exception as e:
                failure_count += 1
...
def parse_all_pdfs_to_csv(pdf_folder: str, output_csv_path: str, verbose: bool = False, summary_mode: bool = False) -> None:
    ...
    After successful processing, copies the PDFs to the target directory.
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract product data from PDF invoices/reports into enhanced and legacy CSV files (v4)."
    )
    parser.add_argument(
        "pdf_folder",
