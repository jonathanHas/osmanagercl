...
class IIHInvoiceParser:
    ...
    def extract_case_size(self, product_desc: str) -> Optional[int]:
        ...
        match = re.search(r'(\d+)x\d+', product_desc, re.IGNORECASE)
        if match:
    ...
    def validate_price_calculation(self, delivered_qty: Dict, case_size: Optional[int], 
        ...
        """
        # If no delivery, value should be 0, but we still calculate unit cost
        ...
        # Method 1: Direct calculation (price is total for quantity)
        if delivered_qty['cases'] > 0 and delivered_qty['units'] == 0:
        ...
        # Method 2: Unit price calculation
        if case_size and case_size > 0:
            # Calculate total units delivered
            total_units = (delivered_qty['cases'] * case_size) + delivered_qty['units']
            
            if total_units > 0:
                # Try: price is per unit
                calculated = total_units * price
                if abs(value - calculated) <= (calculated * tolerance) if calculated > 0 else abs(value) < 0.01:
                    return True, f"{total_units} units × {price:.2f}", calculated, price
                
                # Try: price is per case, calculate unit price
                unit_price = price / case_size
                calculated = total_units * unit_price
                if abs(value - calculated) <= (calculated * tolerance) if calculated > 0 else abs(value) < 0.01:
                    return True, f"{total_units} units × {unit_price:.2f} (case price/{case_size})", calculated, unit_price
        
        # Method 3: Simple quantity (for items without case size)
        simple_qty = delivered_qty['cases'] + delivered_qty['units']
        if simple_qty > 0:
            calculated = simple_qty * price
            if abs(value - calculated) <= (calculated * tolerance) if calculated > 0 else abs(value) < 0.01:
                # For simple quantity, calculate unit cost from value and quantity
                unit_cost = value / simple_qty if simple_qty > 0 else 0
                return True, f"{simple_qty} × {price:.2f}", calculated, unit_cost
        
        # No valid calculation found - try to derive unit cost anyway
        total_units = self.calculate_total_units(delivered_qty, case_size)
        unit_cost = value / total_units if total_units > 0 else 0
                        ...
                        if missing_rsp_match:
                            ...
                            if confidence in ["HIGH", "MEDIUM", "LOW"]:
                                ...
                                if confidence in ["HIGH", "MEDIUM"]:
                                    self.log(f"⚠ Potential miss [{confidence}]: {line[:60]}...", "WARNING")
    ...
    def generate_report(self, all_rows: List[Dict], output_base: str):
        ...
        report_lines = []
        report_lines.append("# IIH Invoice Parsing Report")
...
def main():
    parser = argparse.ArgumentParser(description='Enhanced IIH Invoice Parser with comprehensive feedback')
    parser.add_argument('path', nargs='?', default='.', help='Path to PDF file or directory (default: current directory)')
    parser.add_argument('-o', '--output', default='iih_enhanced', help='Output filename base (default: iih_enhanced)')
