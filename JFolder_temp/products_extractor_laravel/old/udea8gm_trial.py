...
NORMAL_REGEX: Final[Pattern[str]] = re.compile(rf"""
    ...
    ({NUMBER})\s+                   # 7) Price
    ({NUMBER})\s+                   # 8) Sale
    (?:\d+\s+\d+%?\s+)           #    VAT/profit tokens (ignored)
...
# Quick header check
HEADER_REGEX: Final[Pattern[str]] = re.compile(r"^(\d+)\s+(\d+)\s+(\d+)")
...
def _clean_number_string(num_str: str) -> str:
    '''
    Convert European-style number (e.g. '1.234,56' or '2,810') to standard
...
def _parse_line(line: str) -> Optional[Dict[str, str]]:
    ...
    if m:
        ...
        content = f"{unit} {parts[0]}" if parts else unit
        description = parts[1] if len(parts) > 1 else ''
        return { 'Code': code, 'Ordered': ordered, 'Qty': qty,
                 'SKU': sku, 'Content': content, 'Description': description,
                 'Price': _clean_number_string(pr), 'Sale': _clean_number_string(sa), 'Total': _clean_number_string(to) }
    m = QUANTITY_SKU_REGEX.match(line)
    if m:
        code, ordered, qty, aq, sku, unit, desc, pr, sa, to = m.groups()
        content = f"{_clean_number_string(aq)} {unit}"
        return { 'Code': code, 'Ordered': ordered, 'Qty': qty,
                 'SKU': sku, 'Content': content, 'Description': desc,
                 'Price': _clean_number_string(pr), 'Sale': _clean_number_string(sa), 'Total': _clean_number_string(to) }
    ...
    if m:
        ...
        return { 'Code': code, 'Ordered': ordered, 'Qty': qty,
                 'SKU': _clean_number_string(sku), 'Content': content, 'Description': desc,
                 'Price': _clean_number_string(pr), 'Sale': _clean_number_string(sa), 'Total': _clean_number_string(to) }
                ...
                for lnum, raw in enumerate(text.split("\n"), start=1):
                    ...
                    if parsed:
                        ...
                        if tc >= TOKEN_THRESHOLD:
                            logging.warning(f"P{pnum} L{lnum}: high token count ({tc}), probable product—running diagnostics")
                            if DIAGNOSTIC:
                                _diagnose_line(line)
                        else:
...
def main():
    parser = argparse.ArgumentParser(
        description="PDF → CSV parser with threshold-based diagnostics"
    )
    parser.add_argument('pdf_folder', nargs='?', default=os.getcwd(),
                        help='Folder containing PDF files.')
    parser.add_argument('-o','--output', dest='output_csv', default='udea_output.csv',
