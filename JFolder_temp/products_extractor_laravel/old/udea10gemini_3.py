...
def _parse_line(line: str) -> Optional[Dict[str, str]]:
    ...
    if m_qty_sku:
        ...
        return {
            "Code": g_code,
            "Ordered": g_ordered,
            "Qty": g_qty, # This is the Qty used for Qty * Price validation
            "SKU": final_csv_sku,
            "Content": content_output,
            "Description": g_desc,
            "Price": _clean_number_string(g_price), # Price for validation
            "Sale": _clean_number_string(g_sale),
            "Total": _clean_number_string(g_total), # Total for validation
        }
...
def extract_products_from_pdf(pdf_path: str) -> List[Dict[str, str]]:
    ...
    calculation_tolerance = 0.015 # e.g., 1.5 cents

    try:
        with pdfplumber.open(pdf_path) as pdf:
            logging.info(f"Processing PDF: {pdf_filename} ({len(pdf.pages)} pages)")
            for page_num, page in enumerate(pdf.pages, start=1):
                text = page.extract_text(x_tolerance=1, y_tolerance=2) # User's specified x_tolerance
                if not text:
                ...
                for line_num, raw_line in enumerate(text.split("\n"), start=1):
                    ...
                    if parsed:
                        ...
                        # --- Add Qty * Price == Total validation ---
                        try:
                            ...
                            if qty_val_str and price_val_str and total_val_str:
                                qty_f = float(qty_val_str)
                                price_f = float(price_val_str)
                                total_f_pdf = float(total_val_str)

                                expected_total_f = qty_f * price_f

                                # Using math.isclose for robust float comparison is good,
                                # but a manual check with tolerance is also common.
                                # if not math.isclose(expected_total_f, total_f_pdf, rel_tol=1e-5, abs_tol=calculation_tolerance):
                                if abs(expected_total_f - total_f_pdf) > calculation_tolerance:
                                    logging.warning(
                                        ...
                                        f"Qty*Price ({qty_f}*{price_f} = {expected_total_f:.2f}) "
                                        f"does not match Total from PDF ({total_f_pdf:.2f}). "
                                        f"Original line: '{line}'"
                                    )
                            else:
                                logging.debug(f"P{page_num} L{line_num}: Product Code {product_code_for_val} - Missing Qty, Price, or Total for validation.")

                        except ValueError as ve:
                            logging.error(
                                f"P{page_num} L{line_num}: Product Code {parsed.get('Code', 'N/A')} - Could not convert Qty/Price/Total to number for validation: {ve}. Line: '{line}'"
                            )
