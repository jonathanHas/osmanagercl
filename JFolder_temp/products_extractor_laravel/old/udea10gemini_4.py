...
def _parse_line(line: str) -> Optional[Dict[str, str]]:
    ...
    if m_qty_sku:
        ...
        # Determine if the line structure suggests a weight-based product
        is_weight_based_style = "." in cleaned_group4 and "." not in cleaned_group5

        final_csv_sku_value: str # This will be the SKU value stored in CSV, used as multiplier
        quantity_for_content_display: str # This is for the "Content" column display

        if is_weight_based_style:
            # Group4 (e.g., "4.260" weight) is the multiplier (SKU for CSV).
            # Group5 (e.g., "4" pieces) is the quantity for display in "Content" field.
        ...
        # Ensure SKU is at least "1" if it's empty or zero after parsing, to avoid zero multiplication errors
        # if not final_csv_sku_value or float(final_csv_sku_value) == 0:
        #     logging.warning(f"Product {g_code}: Parsed SKU for multiplier is '{final_csv_sku_value}'. Defaulting to '1' for calculation if it was empty/zero.")
        #     final_csv_sku_value = "1" # Default to 1 if SKU is not a valid multiplier
    ...
    if m_fallback:
        ...
        # Ensure SKU is at least "1" if it's empty or zero after parsing
        # if not sku_fb_multiplier or float(sku_fb_multiplier) == 0:
        #     logging.warning(f"Product {code}: Fallback Parsed SKU for multiplier is '{sku_fb_multiplier}'. Defaulting to '1' for calculation if it was empty/zero.")
        #     sku_fb_multiplier = "1"
