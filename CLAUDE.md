...
- **F&V Class Dropdown Not Working** (Fixed 2025-09-04): Users unable to change product class in fruit-veg/manage interface while country dropdown worked fine. **Root Cause**: Validation rule mismatch - `VegClass` model uses primary key `ID` (uppercase) but validation rule checked `exists:App\Models\VegClass,id` (lowercase). **Solution**: Fixed validation rule to use uppercase `ID` and added `id` accessor to VegClass model for template compatibility. Class dropdown now works identically to country dropdown.
- **F&V Unit Dropdown Showing Wrong Options** (Fixed 2025-09-04): Unit dropdown showed 5 options from main database (kg, each, bunch, punnet, bag) when POS database only has 2 (kg, each), causing data inconsistency. **Root Cause**: `VegUnit` model incorrectly referenced main database instead of POS database. **Solution**: Created new `PosUnit` model connecting to POS `units` table, updated `VegDetails` relationship and controller. Now shows only the correct 2 units from POS database, ensuring system consistency.
